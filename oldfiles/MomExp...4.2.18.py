...
fig = plt.figure()
ax = plt.axes(xlim=(0, 500), ylim=(-0.4, 0.4)) #makes your x and y axis, zoom in by changing the values 
...
### Way to calculate slope below
    
def DYDX(fx, x):
    slope= np.zeros(len(xt))
    for g in range(-100, len(xt)):
        if g >= 1:
            slope[g] = ((fx[g])-(fx[g-1])) / (x[g]-(x[g-1]))
        elif g == 0: 
            slope[g] = ((fx[1])-(fx[g])) / (x[1]-(x[g]))
...
integral = 0
for b in range(0, len(Psi)):
    integral = integral + Psi[b]*pderiv[b]*(0.25)

print(integral)
    
#for g in range(0, len(xt)):
#    print(xt[g], Psi[g])
...
plt.plot(pderiv, x, 'green', pderiv2, x, 'brown')
plt.show()
#print(k0)
...
"""
Created on Mon Mar 26 14:29:53 2018
@author: eldabaghr
"""
